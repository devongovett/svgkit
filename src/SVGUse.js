...
class SVGUse extends SVGElement {
  parse() {
    ...
    this.ref_id = this.attributes['xlink:href'].slice(1);
  ...
  draw(ctx, clip = false) {
    ...
    let Element = SVGElement.parsers[def.tagName.toLowerCase()];
    if (!Element) { return; }

    let el = new Element(this.document, this, def.tagName, def.attributes);
    el.parse();
    el.draw(ctx, clip);
